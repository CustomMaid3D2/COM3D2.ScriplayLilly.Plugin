...
namespace GearMenu
{
    ...
    public static class Buttons
    {
        ...
        public static string pluginName = "GearMenu";
        ...
        public static void Log(System.Object s)
        {
            ...
            Console.WriteLine(pluginName + s);
            Console.ResetColor();
            //Debug.Log(s);
        }
        ...
        /// 기어 메뉴 버튼을 추가
        /// </summary>
        /// <param name="plugin">버튼을 추가하는 플러그인. 아이콘에 마우스 오버시 이름과 버전이 표시된다</param>
        /// <param name="pngData">아이콘 이미지. null 가능 (시스템 아이콘 사용). 32x32 픽셀의 PNG 파일</param>
        /// <param name="action">콜백. null 가능 (콜백 삭제). 아이콘 클릭시 호출되는 콜백</param>
        ...
        /// 기어 메뉴 버튼을 추가
        /// </summary>
        /// <param name="name">버튼 객체 명. null 가능</param>
        /// <param name="plugin">버튼을 추가하는 플러그인. 아이콘에 마우스 오버시 이름과 버전이 표시된다</param>
        ...
        public static GameObject Add(string name, string label, byte[] pngData, Action<GameObject> action)
        {
            ...
            try
            {
                ...
                // PNG イメージを設定
                {
                    ...
                    // 사실 스프라이트를 제거하고 싶지만, 삭제하면 패널의 α 값과 Tween 동기화가 작동하지 않습니다
                    // (작동 방법을 모르는) 때문에 스프라이트를 그릴하지 않도록 설정
                    // 원래 가지고 있던 스프라이트를 삭제하려면 다음 코드를 사용하면된다
